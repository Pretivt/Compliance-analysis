...
export const createFrameWork = async (req, res, next) => {
  try {
    ...
    try {
      const chunks = chunkFramework(framework);  // iso framework has 100 controls so we divide it in chunks  chunks 1- iso intro , chunk 2 security controls,  chunk 3 risk policies
      for (const chunk of chunks) {
        const vector = await generateEmbedding(chunk.text);// create vector of framework
        await qdrantClient.upsert("frameworks", { // to store data in qdrant
          wait: true,
          points: [
            {
              id: randomUUID(),
...
// --- PART 1: Sync all existing frameworks to Qdrant ---  .. to find how many framework are there 
