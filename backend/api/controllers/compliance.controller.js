...
export const getReports = async (req, res, next) => {
  try {
    const reports = await Report.find({ createdBy: req.user.id })
      .populate("product", "productName") //  // mongodb usually store the object id of related document not whole data , so populate is used to fetch the details of object id to get into the product 
      .populate("framework", "name shortCode")
      .sort({ createdAt: -1 }); //..newest report
