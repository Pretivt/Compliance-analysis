  ...
  try {
    ...
    return res
      .cookie("token", token, {
        ...
        sameSite: "strict", // cross site request block 
        secure: false,
...
// give data of current logged in user

export const getuser = async (req, res) => {  
  try {
    //  get user
    const user = await Auth.findById(req.user.id).select("-password"); // data from middleware
    if (!user) {
      return res.status(400).json({
